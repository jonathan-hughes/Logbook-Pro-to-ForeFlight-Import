...
def CreateLogBookList():
    ...
    try:
        ...
        for rowLogBook in rawLogBookData:
            ...
            logBookArray.append([lbDate, lbAcID,     lbFrom, lbTo, lbRoute, '',      '',      '',     '',     '',     '',          lbTotalTime, lbPIC,   lbSIC,  lbNight, lbSolo, lbCrossCountry, '',       lbDayTakeOffs, lbDayLandings,       lbNightLandings, lbNightLandings,       int(lbDayLandings) + int(lbNightLandings), lbActualInstrument, lbSimInstrument,     '',         '',       '',        '',      '',    '',           '',           '',           '',           '',           '',           lbDualGiven, lbDualReceived, '',              lbGroundReceived, '',             '',                 '',           '',           '',           '',           '',           '',           '',           '',        '',     lbAerobatics, lbGroundGiven, lbTows, lbXC50NM,    lbLegs, lbPilotComments])
    except Exception as Ex:
        print("Error: " + str(Ex))
